...
Labeled13C=[216.994;171.999;314.973;269.985;402.937;416.978;371.985;502.943;467.980;514.966;469.974;564.962;519.969;614.960;569.968;714.953;669.960;814.947;769.954];
% load('Final_Candidates.mat');
T0=1;                       % scan number of RT=1 min
T_end=11;                   % scan number of RT=11 min
load('Baggedtree.mat');
load('ID_library.mat')
...
for i=1:size(Final_Candidates,1)
    ...
    if isempty(Final_Candidates{i,3}) || Final_Candidates{i,3}==1
        x=find(abs(Labeled13C-Final_Candidates{i,4})<=Delta_mass);
        y=find(abs(Noise_Neg_mz_RT_Blood(:,1)-Final_Candidates{i,4})<=Delta_mass & abs(Noise_Neg_mz_RT_Blood(:,2)-Final_Candidates{i,9})<=Delta_RT);
        if isempty(x) && isempty(y) && ...
                Final_Candidates{i,11}>2 && ...
                Final_Candidates{i,13}<0.7 && ...
                Final_Candidates{i,12}>35 && Final_Candidates{i,12}<120
            %% False positive prediction module
            x_N_Iso=find(ID_M>=ID_I & ID_M<=ID_E);
            N_Iso=XIC_Primary{x_N_Iso,2};       % Number of isotopologues
            S=Baggedtree.predictFcn([N_Iso;       % Number of isotopologues
