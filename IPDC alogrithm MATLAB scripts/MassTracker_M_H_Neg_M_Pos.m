...
clc
mass_Target=214.1223;       % <-- Targted mass (Da)
RT_Target=28.639;           % <-- Targted retention time (min)
...
while i<length(x_Targeted) && k<kmax
    ...
    if A2{8}>2 %&& A2{9}>25 && A2{10}<0.8 && k<kmax
        ...
        x_x_B=find(~cellfun(@isempty,Final_Candidates(x_B,15)));
        B=Final_Candidates(x_B(x_x_B),15:19);
        Counter=Counter+1;
        k=k+1;
...
%     'Peak_Identification_score','Candidate_molecular_formula',...
%     'Exact_mass_of_candidate_compound','Number_of_candidates_in_Comptox','Name'})
