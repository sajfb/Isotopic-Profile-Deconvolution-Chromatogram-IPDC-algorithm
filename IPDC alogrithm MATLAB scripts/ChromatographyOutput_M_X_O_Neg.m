...
% This module returns molecular formulas for [M-X+O]- and [M]- ionization pathways
...
min_NDCS=3;                 % Minimum number of NDCS within each given peak
...
for ID_i=unique(EntireIDs,'stable')'
    ...
    if length(x)>=min_NDCS  % Each candidate compounds should be detected in at least 3 chromatogram scans
        ...
        [No_Isomer, ScanNumber_isomers, PA_isomers, n_w500, RPW, neme, pcs]=PeakArea(Peaks_TIC1,Retention_Time_TIC1,IsotopeModel,Mass_error,...
        ...
        for i=1:No_Isomer
            if length(ScanNumber_isomers{i})>=min_NDCS
                ik=ik+1;
                N_I=N_I+1;
        ...
        if Cond==1
            ...
            if No_Isomer==1
                ...
                Final_Candidates{x_MolF,13}=round(RPW,2);  % Ratio of the peak width at healf height to the peak width at baseline
                Final_Candidates{x_MolF,14}=round((N_Iso^coeff(1))*(log10(Final_Candidates{x_MolF,6}/500)^1)*(log10(Final_Candidates{x_MolF,10}/13.96)^1)...
                    *((Final_Candidates{x_MolF,8}/1000)^coeff(2))*(Final_Candidates{x_MolF,11}^coeff(3))*(Final_Candidates{x_MolF,12}^coeff(4))...
                ...
                for i=1:No_Isomer
                    ...
                    Final_Candidates{x_MolF,13}=round(RPW(i),2);  % Ratio of the peak width at healf height to the peak width at baseline
                    Final_Candidates{x_MolF,14}=round((N_Iso^coeff(1))*(log10(Final_Candidates{x_MolF,6}/500)^1)*(log10(Final_Candidates{x_MolF,10}/13.96)^1)...
                        *((Final_Candidates{x_MolF,8}/1000)^coeff(2))*(Final_Candidates{x_MolF,11}^coeff(3))*(Final_Candidates{x_MolF,12}^coeff(4))...
                        /((Final_Candidates{x_MolF,7}/10)^coeff(5))/(Final_Candidates{x_MolF,13}^coeff(6)),0);
                end
            ...
            %% Fragments in AP- are like [M-X+O]-
            if o>=1
                if cl>0 && br==0
                    molecule=[c,h,br,cl+1,f,i,n,na,o-1,p,s];
                elseif br>0 && cl==0
                    molecule=[c,h,br+1,cl,f,i,n,na,o-1,p,s];
                elseif cl>0 && br>0
                    molecule(1,:)=[c,h,br,cl+1,f,i,n,na,o-1,p,s];
                    molecule(2,:)=[c,h,br+1,cl,f,i,n,na,o-1,p,s];
                end
                for brcl=1:size(molecule,1)
                    br=molecule(brcl,3);cl=molecule(brcl,4);o=molecule(brcl,9);
                    ...
                    if brcl==1
                        MolF=Chemical_structure_Print(molecule(1,:));
                        if No_Isomer==1
                    ...
                    elseif brcl==2
                        if No_Isomer==1
                            RepeatA=Final_Candidates(x_MolF-No_Isomer,1:Column_xlx(1)-1);
                            Final_Candidates(x_MolF+No_Isomer-1,1:Column_xlx(1)-1)=RepeatA;
                            x_MolF=x_MolF+1;
                        else
                            RepeatA=Final_Candidates(x_MolF-No_Isomer-1:x_MolF-1,1:Column_xlx(1)-1);
                            Final_Candidates(x_MolF:x_MolF+No_Isomer,1:Column_xlx(1)-1)=RepeatA;
                            x_MolF=x_MolF+No_Isomer+1;
                        end
                        MolF=Chemical_structure_Print(molecule(2,:));
                        if No_Isomer==1
                            Final_Candidates{x_MolF-No_Isomer,Column_xlx(1)}=MolF;
                            Final_Candidates{x_MolF-No_Isomer,Column_xlx(1)+1}=Mass;
                        else
                            Final_Candidates{x_MolF-No_Isomer-1,Column_xlx(1)}=MolF;
                            Final_Candidates{x_MolF-No_Isomer-1,Column_xlx(1)+1}=Mass;
                        end
                    end
                end
            else
            ...
            %%     Fragments in AP- are like [M]-
            Mass=Final_Matches(x(1),3);
            MolF=Chemical_structure_Print(MolecularIon);
            if No_Isomer==1
                Final_Candidates{x_MolF-1,Column_xlx(2)}=MolF;
                Final_Candidates{x_MolF-1,Column_xlx(2)+1}=Mass;
            else
                Final_Candidates{x_MolF-No_Isomer-1,Column_xlx(2)}=MolF;
                Final_Candidates{x_MolF-No_Isomer-1,Column_xlx(2)+1}=Mass;
            end
            %%
...
k=1;
splitxls=500000; % If Excel does not allow a big spreadhsset, you may split it into multiple spreadsheets. "splitxls" shows the number of rows in each spreadsheet.
while k<=size(Final_Candidates,1)
    ...
    xlswrite(namexls,{'ID in MATLAB code','Candidate molecular ion formula',...
        ...
        'Candidate molecular formula [M-X+O]-','Exact mass of candidate compound  [M-X+O]- (Da)',...
        'EPA DSSTox','Name','Candidate molecular formula [M]-',...
        'Exact mass of candidate compound  [M]- (Da)','EPA DSSTox','Name','Link'});
