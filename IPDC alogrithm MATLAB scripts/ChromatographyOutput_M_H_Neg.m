...
format longG
route='C:\Users\ACD\Desktop\APGC\';     % the address of essential files
    ...
    if length(x)>=min_NDCS  % Each candidate compounds should be detected in at least 3 chromatogram scans
        ...
        if Cond==1
            ...
            if No_Isomer==1
                ...
                Final_Candidates{x_MolF,14}=round((N_Iso^coeff(1))*(log10(Final_Candidates{x_MolF,6}/min_INT)^1)*...
            ...
            %% Fragments in AP- are like [M-H]-
            MolecularIon=ID_library(ID_i,:);
            c=MolecularIon(1);h=MolecularIon(2);br=MolecularIon(3);cl=MolecularIon(4);f=MolecularIon(5);
            i=MolecularIon(6);n=MolecularIon(7);na=MolecularIon(8);o=MolecularIon(9);p=MolecularIon(10);
            s=MolecularIon(11);
            Mass=Final_Matches(x(y(1)),3)+1.00783;
            MolF=Chemical_structure_Print([c,h+1,br,cl,f,i,n,na,o,p,s]);
            if No_Isomer==1
                Final_Candidates{x_MolF,15}=MolF;
                Final_Candidates{x_MolF,16}=Mass;
            else
                Final_Candidates{x_MolF-No_Isomer,15}=MolF;
                Final_Candidates{x_MolF-No_Isomer,16}=Mass;
            end
            x_MolF=x_MolF+1;
            %%
...
for i=1:size(Final_Candidates,1)
    if ~isempty(Final_Candidates{i,3})
        if Final_Candidates{i,11}>=2*Final_Candidates{i,3}+1
            i
            for n_f=1:1         % n_f represent the number of candidate molecular adduct types
...
Final_Candidates=Known_PFAS_Search(Final_Candidates,Column_xlx);
%%
...
while k<=size(Final_Candidates,1)
    ...
    xlswrite(namexls,{'ID in MATLAB code','Candidate molecular ion formula',...
        ...
        'Retention time (min)','Peak Area','Number of chromatogram scan(s)','RCS(%)',...
        'RPW','Peak Identification score',...
        'Candidate molecular formula [M-H]-','Exact mass of candidate compound  [M-H]- (Da)',...
        'EPA DSSTox','Name','Link'});
    if k+splitxls<=size(Final_Candidates,1)
        xlswrite(namexls,Final_Candidates(k:k+splitxls,:),['A2:R',num2str(splitxls+1)])
    else
        xlswrite(namexls,Final_Candidates(k:size(Final_Candidates,1),:),['A2:R',num2str(size(Final_Candidates,1)-k+1)])
    end
    k=k+splitxls
end
%% To create a folder containing of compounds that have at least one record in comptox
Chromatogram_Comptox_DSSTox_1
